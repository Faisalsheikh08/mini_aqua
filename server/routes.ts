...
// import { searchRequestSchema, insertQuestionSchema, updateQuestionSchema, categoryUpdateSchema, bulkCategoryUpdateSchema, type InsertQuestion } from "../shared/schema.js";
...
//   app.post("/api/questions/upload", upload.single('csvFile'), async (req: MulterRequest, res) => {
//     try {
//       if (!req.file) {
...
//       const createdQuestions = await storage.bulkCreateQuestions(questions, (processed, total) => {
...
//       console.log(`Successfully inserted ${createdQuestions.length} new questions`);
...
//           const newConfigIds = [...new Set(createdQuestions.map(q => q.configId))];
//           console.log(`Found ${newConfigIds.length} unique config IDs in uploaded data`);
...
//                 const primaryQuestion = duplicates.find(q => 
//                   detectLanguageFromContent(q.question) === "English"
//                 ) || duplicates[0];
                
//                 const otherVariants = languages.filter(l => l.questionId !== primaryQuestion.questionId);
...
//                 uploadProgress.progress = 80 + Math.round((i / newConfigIds.length) * 15); // 80-95%
//                 uploadProgress.stage = `Merging progress: ${mergeResults.totalMerged} sets merged`;
//               }
//             } catch (mergeError) {
//               console.warn(`Error merging config ID ${configId}:`, mergeError);
//               mergeResults.errors++;
//             }
//           }
          
//           console.log(`Merge process completed: ${mergeResults.totalMerged} merged, ${mergeResults.errors} errors`);
...
//       uploadProgress.isActive = false;
//       res.status(500).json({ message: "Failed to upload CSV", error: (error as Error).message });
//     }
//   });
...
//       const categoryUpdate = categoryUpdateSchema.parse({ questionId, ...req.body });
//       const updatedQuestion = await storage.updateQuestionCategory(categoryUpdate);
...
//       const fileName = `${testName.replace(/[^a-zA-Z0-9]/g, '_')}_Questions.docx`;
...
//         return res.status(400).json({ message: "Invalid merge request parameters" });
//       }
      
//       const result = await storage.mergeMultilingualQuestions(configId, primaryQuestionId, languageVariants);
//       res.json({ 
...
//       console.log(`Found ${duplicateConfigIds.length} config IDs with duplicates`);
...
//       for (let i = 0; i < Math.min(duplicateConfigIds.length, 100); i += batchSize) {
//         const batch = duplicateConfigIds.slice(i, i + batchSize);
//         console.log(`Processing batch ${Math.floor(i/batchSize) + 1} (${batch.length} config IDs)...`);
...
//                 console.log(`Merged ${questions.length} variants for config ${configId}`);
//               }
//             }
//           } catch (error: any) {
//             const errorMsg = `Error merging config ${configId}: ${error?.message || 'Unknown error'}`;
...
//       res.status(500).json({ message: "Multilingual merge failed", error: error?.message || 'Unknown error' });
//     }
//   });
...
//       console.log("This will separate unrelated questions that share config IDs...");
...
//       for (const configId of duplicateConfigIds.slice(0, 100)) { // Process in batches
//         try {
//           const questions = await storage.getQuestionsByConfigId(configId);
          
//           if (questions.length > 1) {
//             // Check if questions are actually different (not just multilingual variants)
//             const areQuestionsDifferent = checkIfQuestionsAreDifferent(questions);
            
//             if (areQuestionsDifferent) {
//               // These are different questions sharing the same config ID - separate them
//               for (let i = 1; i < questions.length; i++) {
//                 const newConfigId = `${Date.now()}_${i}_${Math.random().toString(36).substr(2, 9)}`;
//                 await storage.updateQuestionConfigId(questions[i].id, newConfigId);
//                 separatedQuestions++;
//                 console.log(`Separated question ${questions[i].id} with new config ID: ${newConfigId}`);
//               }
//             }
//           }
...
//             console.log(`Processed ${processedConfigs}/${Math.min(duplicateConfigIds.length, 100)} config IDs...`);
//           }
//         } catch (error: any) {
//           const errorMsg = `Error processing config ID ${configId}: ${error?.message || 'Unknown error'}`;
//           console.error(errorMsg);
//           errors.push(errorMsg);
//         }
//       }
...
//       res.status(500).json({ message: "Smart duplicate fix failed", error: error?.message || 'Unknown error' });
//     }
//   });
...
//         return res.status(409).json({ message: "Bulk export already in progress" });
//       }
...
//         return res.status(404).json({ message: "No bulk export file available" });
//       }
...
//             const safeFileName = testName.replace(/[^a-zA-Z0-9\s\-_]/g, '_')
//               .replace(/\s+/g, ' ')
//               .replace(/_+/g, '_')
//               .trim();
//             const filePath = path.join(tempDir, `${safeFileName} Questions.docx`);
...
//                 bulkExportProgress.completedFiles.push(`${safeFileName} Questions.docx`);
//                 console.log(`Created: ${safeFileName} Questions.docx (${buffer.length} bytes)`);
//               } else {
...
//         bulkExportProgress.progress = Math.round(((i + 1) / testNames.length) * 80);
...
//       console.log(`Starting ZIP creation with ${createdFiles.length} files using yazl...`);
...
//       bulkExportProgress.isActive = false;
//       bulkExportProgress.stage = `Export failed: ${error instanceof Error ? error.message : 'Unknown error'}`;
//     }
//   }
...
//       const result = await duplicateCleanupService.performComprehensiveCleanup();
...
//         duplicateAnalysisProgress.progress = 30 + Math.round((i / duplicateConfigIds.length) * 60);
//         duplicateAnalysisProgress.stage = `Analyzed ${i + 1}/${duplicateConfigIds.length} groups...`;
//       }
...
//     const questionText = q.question?.replace(/<[^>]*>/g, '').replace(/&[^;]+;/g, '').toLowerCase() || '';
//     const options = [q.option1, q.option2, q.option3, q.option4]
//       .filter(Boolean)
//       .map(opt => opt?.replace(/<[^>]*>/g, '').replace(/&[^;]+;/g, '').toLowerCase())
//       .join(' ');
...
//       if (similarity < 0.3) {
//         console.log(`Found different questions in config ${questions[0].configId}: similarity = ${similarity.toFixed(2)}`);
...
//     const questionText = q.question?.replace(/<[^>]*>/g, '').toLowerCase() || '';
...
//     [q.option1, q.option2, q.option3, q.option4, q.option5].filter(Boolean).length
//   );
...
//     const aIsEnglish = /^[a-zA-Z\s.,!?;:()0-9-]+$/.test(a.question?.replace(/<[^>]*>/g, '') || '');
//     const bIsEnglish = /^[a-zA-Z\s.,!?;:()0-9-]+$/.test(b.question?.replace(/<[^>]*>/g, '') || '');
...
//         .replace(/<img[^>]*(?:width|height)=["']?(?:[1-9]|[1-4][0-9]|50)["']?[^>]*>/gi, '')
//         // Remove images with icon-related src attributes
//         .replace(/<img[^>]*src=["'][^"']*(?:icon|key|document|check|mark|circle)[^"']*["'][^>]*>/gi, '')
//         // Remove images with icon-related alt text
//         .replace(/<img[^>]*alt=["'][^"']*(?:icon|key|document|check|mark)[^"']*["'][^>]*>/gi, '')
//         // Remove images with icon-related class names
//         .replace(/<img[^>]*class=["'][^"']*(?:icon|key|document|check|mark|circle|rounded-full)[^"']*["'][^>]*>/gi, '')
//         // Remove common small icon formats
//         .replace(/<img[^>]*src=["'][^"']*\.(?:svg|png|jpg|jpeg)["'][^>]*(?:width|height)=["']?(?:[1-9]|[1-4][0-9]|50)["']?[^>]*>/gi, '')
//         // Remove standalone icon tags
//         .replace(/<img[^>]*\b(?:width|height)=["']?(?:[1-9]|[1-4][0-9]|50)["']?[^>]*\b(?:width|height)=["']?(?:[1-9]|[1-4][0-9]|50)["']?[^>]*>/gi, '');
//     }
...
//               subject: currentRecord[11] ? processHtmlContent(currentRecord[11]) : undefined,
//               topic: currentRecord[12] ? processHtmlContent(currentRecord[12]) : undefined,
//               difficulty: currentRecord[13] ? processHtmlContent(currentRecord[13]) : undefined,
//               questionType: currentRecord[14] ? processHtmlContent(currentRecord[14]) : undefined,
//               category: currentRecord[15] ? processHtmlContent(currentRecord[15]) : undefined,
//               subCategory: currentRecord[16] ? processHtmlContent(currentRecord[16]) : undefined,
//               tags: currentRecord[17] ? currentRecord[17].split(',').map(tag => tag.trim()).filter(tag => tag) : undefined,
//             };
...
//               }
//             } else {
//               console.warn(`Skipping invalid question at record ${recordCount + 1}: missing essential fields`);
//             }
//           } catch (error) {
...
//   console.log(`Parsed ${questions.length} questions from ${recordCount} total records`);
//   return questions;
// }
...
//   const clean1 = text1.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase();
//   const clean2 = text2.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase();
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/export/selected", async (req, res) => {
    try {
      ...
      // Create Word document
      const documentTitle = title || `Selected Questions Export - ${new Date().toLocaleDateString()}`;
      ...
      // Set headers for file download
      const fileName = `selected-questions-${new Date().toISOString().split('T')[0]}.docx`;
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
      res.setHeader('Content-Disposition', `attachment; filename="${fileName}"`);
      res.setHeader('Content-Length', buffer.length);
